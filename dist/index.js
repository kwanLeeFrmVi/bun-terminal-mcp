...
function createErrorMessage(command, exitCode, stderr, errorType) {
  const messages = {
...
server.registerTool("execute_command", {
  ...
  inputSchema: {
    command: exports_external2.string().describe("Shell command string. Supports pipes, redirects, globs, variables, command substitution. Examples: 'ls -la', 'cat file.txt | grep pattern', 'echo $HOME'"),
    timeout: exports_external2.number().optional().describe("Optional timeout in milliseconds (default: 30000ms / 30s, max: 300000ms / 5min)")
  }
...
\uD83D\uDCA1 Verify the file path exists.`;
    ...
    if (useJsonOutput) {
      const response = {
        ...
        errorMessage: isTimeout ? `Command exceeded time limit of ${timeoutMs}ms (${timeoutMs / 1000}s).` : `Failed to execute command: ${err.message}`,
        hint: isTimeout ? "Command took too long to execute. Try increasing timeout or simplifying the command." : "This is a shell execution error. Check command syntax and shell compatibility.",
